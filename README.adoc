...
First you set up a basic build script. You can use your preferred build system when building apps with Spring, but the code you need to work with when using http://gradle.org[Gradle] is included here. If you're not familiar with it, refer to link:/guides/gs/gradle[Building Java Projects with Gradle]. We also provide a Maven pom.xml 
...
`HelloPojo` class is a simple `POJO` in a sense that it doesn't extend any Spring YARN base classes. What we did in this class:
 ...
 - We added a class level {YarnContainerAnnotation}[`@YarnContainer`] annotation.
 - We added a method level {OnYarnContainerStartAnnotation}[`@OnYarnContainerStart`] annotation
 - We `@Autowired` a Hadoop's `Configuration` class

{YarnContainerAnnotation}[`@YarnContainer`] is a stereotype annotation, providing a Spring {Component}[`@Component`] annotation. This is automatically marking a class to be a candidate for having {YarnContainerAnnotation}[`@YarnContainer`] functionality.

Within this class we can use {OnYarnContainerStartAnnotation}[`@OnYarnContainerStart`] annotation to mark a public method with `void` return type and no arguments act as an entry point for some application code that needs to be executed on Hadoop.
...
To accomplish this, simply run your executable client jar.

[source]
----
$ cd gs-yarn-basic-dist
$ java -jar target/gs-yarn-basic-dist/gs-yarn-basic-client-0.1.0.jar
----
